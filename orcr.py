...
def optimize_column_rebar(
    ...
    if model.getNSols() > 0:
        ...
        b_nt_result = round((b - 2*pt_result - 2*cd_result - 2*stirrup_d - bn_result*bd_result)/(bn_result + 1))
        h_nt_result = round((h - 2*pt_result - 2*cd_result - 2*stirrup_d - hn_result*hd_result)/(hn_result + 1))
        #print('结果:')
        #print('全部配筋梁(cm2):', obj)
